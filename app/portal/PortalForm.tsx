...
export default function PortalForm({ company, baseEmployees, alreadySubmitted, reportMonth, lastReport, rejectReason }: PortalFormProps) {
    ...
    return (
        <div className="space-y-4 mt-6">
            {rejectReason && (
                <div className="bg-red-50 border-2 border-red-200 rounded-xl p-5 flex gap-3 items-start animate-in fade-in duration-300">
                    <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center shrink-0">
                        <span className="text-red-600 text-lg font-bold">!</span>
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-red-800 mb-1">您的报告已被退回，请修改后重新提交</h3>
                        <p className="text-sm text-red-700 leading-relaxed">退回原因：{rejectReason}</p>
                    </div>
                </div>
            )}
