...
import { X, Factory, TrendingUp, TrendingDown, Users, MapPin } from 'lucide-react';
import {
    ResponsiveContainer,
    ComposedChart,
    Bar,
                        ...
                        <div className="flex items-start justify-between mb-4 md:mb-6">
                            <div className="min-w-0 flex-1">
                                <h2 className="text-lg md:text-2xl font-bold text-slate-900 flex items-center gap-2 md:gap-3">
                                    <Factory className="w-5 h-5 md:w-6 md:h-6 text-indigo-500 shrink-0" />
                                    <span className="truncate">{data.name}</span>
                                </h2>
                                <span className="inline-block mt-1.5 md:mt-2 px-2 md:px-3 py-0.5 md:py-1 bg-indigo-50 text-indigo-700 text-xs md:text-sm font-medium rounded-full">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <ComposedChart data={data.monthlyTrend} margin={chartMargin}>
                                        ...
                                        <Line
                                            ...
                                            stroke="#34d399"
                                            strokeWidth={2}
                                            dot={isMobile ? false : { r: 4, fill: '#fff', stroke: '#34d399', strokeWidth: 2 }}
                                            activeDot={{ r: 5 }}
                                        />
                                    </ComposedChart>
                                </ResponsiveContainer>
                                    ...
                                    {data.topCompanies.map((comp, idx) => (
                                        <div key={comp.name} className="bg-slate-50 rounded-lg p-3">
                                            <button
                                                ...
                                                className="text-sm font-medium text-indigo-600 hover:underline text-left truncate block w-full"
                                            >
                                                {idx + 1}. {comp.name}
                                            </button>
                        ...
                        {data.townDistribution.length > 0 && (
                            <div className="bg-slate-50 rounded-xl p-3 md:p-4">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {data.townDistribution.map((t) => (
                                        <span key={t.town} className="inline-flex items-center gap-1 text-xs bg-white px-2 py-1 rounded-md border border-slate-200">
...
function KpiCard({ label, value, unit, color, compact }: { label: string; value: string; unit?: string; color: string; compact?: boolean }) {
    return (
